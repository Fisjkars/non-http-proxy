...
public class LogEntry {
    ...
    public long save() {
        ...
        RequestEntity dao = new RequestEntity(0, this.requestResponse, this.original, this.SrcIP, this.SrcPort, this.DstIP, this.DstPort, this.Direction, this.time.getTime(), this.Bytes);
    ...
    public long saveCommitDeferred(LogEntry le) {
        RequestEntity dao = new RequestEntity(0, le.requestResponse, le.original, le.SrcIP, le.SrcPort, le.DstIP, le.DstPort, le.Direction, le.time.getTime(), le.Bytes);
        session.save(dao);
        this.Index = (long) dao.getId();
        return dao.getId();
    ...
    public RequestEntity getData(Long index) {
        RequestEntity r = new RequestEntity();
        Session s = SessionFactorySingleton.getSessionFactory().openSession();
        try {

            r = (RequestEntity) s.createQuery("from Requests where id = :id").setInteger("id", index.intValue()).uniqueResult();
